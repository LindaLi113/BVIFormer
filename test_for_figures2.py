...
def find_matching_gt(hazy_path: str, gt_dir: str):
    ...
    m = re.match(r'^(\d+)', stem)
    base = m.group(1) if m else stem.split('_')[0]
...
# ---------------------------
# (kept) match spatial for tiled path
# ---------------------------
...
# ---------------------------
# (old) tiled inference (kept but not used in full mode)
# ---------------------------
def forward_tiled_parallel(
    ...
    with torch.cuda.amp.autocast(dtype=torch.bfloat16, enabled=(amp_inference and img.is_cuda)):
...
# ---------------------------
# NEW: full-image inference with optional padding
# ---------------------------
def _pad_to_mod(x: torch.Tensor, mod: int, pad_mode: str = "reflect"):
    """
    ...
    # reflect 对小图/大pad可能报错，自动降级到 replicate 更稳
    mode = pad_mode
    if mode == "reflect" and (H <= 1 or W <= 1 or pad_h >= H or pad_w >= W):
        mode = "replicate"

    if mode == "constant":
        x = F.pad(x, (0, pad_w, 0, pad_h), mode=mode, value=0.0)
    else:
...
def forward_full_image(model, img, amp_inference=False, pad_mod=1, pad_mode="reflect"):
    """
    Whole-image inference once, with optional pad-to-multiple and crop-back.
    """
    B, C, H, W = img.shape
    assert B == 1, "Only support batch=1 input image"

    x, (pad_h, pad_w) = _pad_to_mod(img, pad_mod, pad_mode)
...
def main():
    ...
    # NEW: full-image padding options
    ...
    # (kept) tiled args in case you later want to compare
